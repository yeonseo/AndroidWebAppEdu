			...
			if (req.readyState() == 4 && req.status() == 200) {
					...
					if (afterAction != null){
						for(var i = 0 ; i < afterAction.length; i++){
							if(typeof afterAction[i] === "function") {
								console.log(afterAction[i]);
								afterAction[i]();
							}
 ...
 * @param curOrder 현재위치
 */
function prevAdType(adName, targetId, curOrder, exposePage, bannerCurOrder, bannerExposePage, afterFunction) {
	if(shopadbox.status.isFlicking()){
		_prevAdTypeForFlicking();		//플리킹일시 해당 함수 호출
	}else{
		// 트렌드 몰은 배너가 없어서 배너관련 파라미터를 전송하지 않는다.
		if (!bannerCurOrder && !bannerExposePage) {
			sendAjaxAfterInnerHtml(tabTitle[adName],{type:'prev', order:curOrder, exposePage:exposePage}, targetId, afterFunction);
		} else {
			sendAjaxAfterInnerHtml(tabTitle[adName],{type:'prev', order:curOrder, exposePage:exposePage, bannerType:'prev', bannerOrder:bannerCurOrder, bannerExposePage:bannerExposePage}, targetId, afterFunction);
		}
	}
}
 ...
 * @param curOrder 현재위치
 */
function nextAdType(adName,targetId,curOrder,exposePage, bannerCurOrder, bannerExposePage, afterFunction){
	if(shopadbox.status.isFlicking()){
		_nextAdTypeForFlicking();		//플리킹일시 해당 함수 호출
	}else {
		// 트렌드 몰은 배너가 없어서 배너관련 파라미터를 전송하지 않는다.
		if (!bannerCurOrder && !bannerExposePage) {
			sendAjaxAfterInnerHtml(tabTitle[adName],{type:'next', order:curOrder, exposePage:exposePage}, targetId, afterFunction);
		} else {
			sendAjaxAfterInnerHtml(tabTitle[adName],{type:'next', order:curOrder, exposePage:exposePage, bannerType:'next', bannerOrder:bannerCurOrder, bannerExposePage:bannerExposePage}, targetId, afterFunction);
		}
	}
}
 ...
 * @param curOrder 현재위치
 */
function nextAdBanner(adName, targetId, bannerCurOrder, bannerExposePage, afterFunction) {
	sendAjaxAfterInnerHtml(tabTitle[adName], {bannerType:'next', bannerOrder:bannerCurOrder, bannerExposePage:bannerExposePage}, targetId, afterFunction);
}
